...
class FileContent:
    ...
    def from_bits(cls, bit_sequence):
        ...
        text_data = (
            int(bit_sequence, 2)
            .to_bytes((len(bit_sequence) + 7) // 8, byteorder="big")
            .decode("utf-8")
        )
...
class StopAndWaitMessage:
    ...
    def from_bits(cls, bit_sequence):
        ...
        sq_number_bits = bit_sequence[
            SEQUENCENUMBERBITLENGTH
        ]  # Assuming SequenceNumber is 1 bit
        content_bits = bit_sequence[
            SEQUENCENUMBERBITLENGTH:
        ]  # Assuming the rest is for FileContent
