...
def main(addr: tuple[str, int]):
    ...
    Note:
        ...
        It then receives and prints the server's response until 
        the server signals the end of the listing process.
    """
    client_socket = RdtSWSocketClient()  # Initialize the client socket
    command = Command(
        MessageOption.LIST_FILES, None, 0
    )  # Create a command to list files

    while True:
        client_socket.send_with_internal_socket(
            command.to_str().encode(), addr
        ...
        if (
            LIST_FILES_FINISH in response.decode()
        ):  # If the server signals the end of listing
            break
        else:
...
if __name__ == "__main__":
    ...
    if not parser_utils.verify_params(
        args, "ls"
        ...
        # Call the main function with server address
