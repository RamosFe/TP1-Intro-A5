...
def download_file(
    ...
    Args:
        ...
        socket_to_client (socket.socket): The socket to send responses
        to the client.
        addr (tuple[str, int]): The address of the client.
        mount_path (str): The path to the directory where files
        will be downloaded.
        exit_signal (threading.Event): An event signaling whether
        to exit the download process.
        comm (Command): The command object containing information
        about the download.

    Note:
        This function sends responses to the client indicating whether
        the download can proceed or not.
        If the file already exists in the download directory, it sends
        an error response.
        If the file does not exist, it sends an OK response
    ...
    if socketSW is not None:
        ...
        try:
            if fs_handler.file_exists(filename=comm.name):
                if verbose:
                    print(
                        f"[{addr[0]}:{addr[1]}] File {comm.name} already exists, cancelling upload ❌"
                    )
                ...
                three_way_handshake.send_with_queue(response, addr, channel)
                fs_handler.download_file(
                    channel, socketSW, None, comm.name, exit_signal
                )
        except TimeoutError:
            return
    else:
        ...
        if fs_handler.file_exists(filename=comm.name):
            ...
            socketSR.send_message(response.encode())
            fs_handler.download_file(
                channel, socketSW, socketSR, comm.name, exit_signal
            )
